...
export default function AuditLog({ sessionId, refreshKey }: { sessionId: string; refreshKey: number }) {
  ...
  useEffect(() => {
    if (!open) return;
    api
      .get<AuditRow[]>(`/api/v1/sessions/${sessionId}/audit?limit=100`)
      .then(setRows)
  ...
  return (
    <section className="mt-8">
      <button onClick={() => setOpen(!open)} className="font-semibold text-slate-300 hover:text-slate-100">
        {open ? "▾" : "▸"} Nhật ký hoạt động
      </button>
      {open && (
        <div className="mt-2 max-h-72 overflow-y-auto rounded-lg border border-slate-800 bg-slate-900">
